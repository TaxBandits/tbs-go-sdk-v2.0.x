...
// AuthService obtains and caches TaxBandits OAuth 2.0 access tokens.
type AuthService interface {
	// GetJWT returns a cached or freshly-issued access token for the given
	// scope/forms. Empty scope/forms fall back to the last-used values.
	// If isUnauthorised is true, it forces a "FullAccess"/"All" cache key,
	// used to recover from an upstream 401.
...
// NewAuthService builds an AuthService that issues requests via client
// and authenticates using cfg's OAuth credentials. A nil client defaults
// to a 30-second-timeout *http.Client.
...
func (s *authService) GetJWT(ctx context.Context, scope string, forms []string, isUnauthorised bool) (string, error) {
	...
	// An unauthorized response must fetch a new token rather than returning
	// the cached FullAccess/All token that just failed.
	if !isUnauthorised {
...
func (s *authService) requestToken(ctx context.Context, jwsToken string) (string, error) {
	...
	payload, err := httphelper.DecodeResponseBody(response.Body)
	if err != nil {
		return "", err
	}
